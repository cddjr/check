...
class WYXW:
    ...
    def sign(self):
        headers = {
            ...
            "User-tk": self.user_tk,
            "User-D": self.user_d,
            "User-id": self.user_id,
        }
        response = requests.post(url="https://c.m.163.com/uc/api/sign/v3/commit", headers=headers, data=self.data, verify=False).json()
        try:
    ...
    def main(self):
        try:
            ...
            self.user_id = self.check_item.get("user_id")
            self.user_d = self.check_item.get("user_d")
            self.user_tk = self.check_item.get("user_tk")
